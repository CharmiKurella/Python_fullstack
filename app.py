...
def get_tasks():
    ...
    if not user_email:
        ...
        "message":"No user logged in!,Login please!"}), 401
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * ROM tasks WHERE user_email = ?", (user_email,))
...
def add_task():
    ...
    user_email = sessions.get("user_email")
    if not user_email:
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO taks (user_email, title) VALUES (?, ?)", (user_email, title))
    conn.commit()
    conn.close()
...
def delete_tasks(task_id):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM tasks WHERE user_email=?,(user_email,task_id))")
    conn.commit()
    conn.close()
